...
<div x-data="{
    ...
    {{-- Hero layer — centred near the top, above everything. --}}
    <div class="pointer-events-none fixed inset-x-0 top-0 z-[95] flex justify-center px-4 pt-6 sm:pt-10">
        <template x-if="hero">
            <div class="nx-hero pointer-events-auto"
                ...
                <div class="nx-hero__glyph" :class="'nx-hero__glyph--' + hero.type" aria-hidden="true">
                    {{-- success: drawn check --}}
                    <svg x-show="hero.type === 'success'" class="nx-hero__draw" viewBox="0 0 52 52"><circle class="nx-hero__ring" cx="26" cy="26" r="24" fill="none" stroke-width="2"/><path class="nx-hero__tick" fill="none" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" d="M14 27l8 8 16-18"/></svg>
                    {{-- error: cross --}}
                    <svg x-show="hero.type === 'error'" class="nx-hero__draw" viewBox="0 0 52 52"><circle class="nx-hero__ring" cx="26" cy="26" r="24" fill="none" stroke-width="2"/><path class="nx-hero__tick" fill="none" stroke-width="4" stroke-linecap="round" d="M18 18l16 16M34 18L18 34"/></svg>
                    {{-- pending: spinner --}}
                    <svg x-show="hero.type === 'pending'" class="nx-hero__spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                    {{-- info --}}
                    <svg x-show="hero.type === 'info'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="h-7 w-7"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>
                </div>
    ...
    {{-- Corner layer — light confirmations. --}}
    <div class="pointer-events-none fixed bottom-4 right-4 z-[90] flex flex-col items-end gap-2" aria-live="polite">
