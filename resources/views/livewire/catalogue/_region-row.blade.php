...
<button type="button" wire:click="openRegion('{{ $t['slug'] }}')"
        class="group flex items-center gap-3 rounded-2xl border border-slate-200 nx-glass-tile p-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary/40 dark:border-[#2D4060]">
    <span class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-slate-100 dark:bg-[#152238]">
        @if (! empty($t['icon']))
