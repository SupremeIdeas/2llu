<div class="mx-auto max-w-2xl px-4 py-6">
    @php($input = 'w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-[#2D4060] dark:bg-[#243352] dark:text-slate-100')
    <div class="mb-5">
    ...
    <div class="mb-5 rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-[#16233d]">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-3">
                ...
                <input type="color" wire:model="profile.background_color" class="h-9 w-14 rounded border border-slate-300 dark:border-[#2D4060]">
    ...
    <div class="mb-5 rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-[#16233d]">
        ...
        <div x-data="{ p: @entangle('newHandle.platform') }" class="mt-3 grid gap-2 sm:grid-cols-4">
            ...
            <div class="sm:col-span-4">
                ...
                <button wire:click="addHandle" class="mt-1 rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 dark:border-[#2D4060] dark:text-slate-200">Add handle</button>
            </div>
        </div>
    </div>
    ...
    @if (($brand->plan?->video_previews_allowed ?? 0) > 0)
        <div class="mb-5 rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-[#16233d]">
            ...
            <div class="mt-3 grid gap-2 sm:grid-cols-4">
                ...
                <button wire:click="addVideo" class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 dark:border-[#2D4060] dark:text-slate-200">Add video</button>
            </div>
        </div>
    @endif
        ...
        <div class="flex flex-wrap gap-2">
            @foreach ($plans as $plan)
                <button wire:click="changePlan({{ $plan->id }})"
                        class="rounded-full border px-3.5 py-1.5 text-xs font-semibold transition {{ $brand->current_plan_id === $plan->id ? 'border-primary bg-primary/10 text-primary dark:text-teal-300' : 'border-slate-200 text-slate-600 hover:bg-slate-50 dark:border-[#2D4060] dark:text-slate-300' }}">
