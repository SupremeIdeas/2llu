            ...
            <div class="flex flex-wrap gap-2">
                @foreach ($allScopes as $scope)
                    <label class="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition
                        {{ in_array($scope, $scopes, true) ? 'border-primary bg-primary/10 text-primary-dark dark:text-primary' : 'border-slate-200 text-slate-600 dark:border-[#2D4060] dark:text-slate-300' }}">
        ...
        @forelse ($clients as $client)
            <div class="rounded-2xl border border-slate-200 nx-glass-tile p-4 shadow-sm dark:border-[#2D4060]" wire:key="client-{{ $client->id }}">
                ...
                @if ($client->is_active)
                    <div class="mt-3 flex flex-wrap items-end gap-2">
                        <div class="flex-1">
                            ...
                            <input type="number" min="1" step="0.01" wire:model="topUp.{{ $client->id }}" placeholder="10.00"
                                   class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 dark:border-[#2D4060] dark:bg-[#1A2840] dark:text-slate-100">
                        </div>
                    ...
                    <div class="mt-3 flex items-center gap-3 border-t border-slate-100 pt-3 text-xs dark:border-[#2D4060]">
