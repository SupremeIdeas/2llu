...
<div class="mb-4 flex items-stretch gap-2">
    ...
    <button type="button" wire:click="browseCountries" aria-label="Browse by country"
            class="flex shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-white px-3.5 text-primary shadow-sm transition hover:bg-slate-50 dark:border-[#2D4060] dark:bg-[#1A2840] dark:text-teal-300 dark:hover:bg-[#243352]">
...
<div class="mb-4 grid grid-cols-2 gap-1 rounded-2xl border border-slate-200 bg-slate-100 p-1 dark:border-[#2D4060] dark:bg-[#1A2840]">
...
<div class="-mx-4 mb-5 flex gap-2 overflow-x-auto px-4 pb-1 sm:mx-0 sm:px-0" style="scrollbar-width: none;">
    @foreach ($chips as [$key, $label, $icon])
        <button wire:click="setView('{{ $key }}')" @class([
            ...
            'border-slate-200 bg-white text-slate-600 hover:border-primary/40 hover:text-primary dark:border-[#2D4060] dark:bg-[#1A2840] dark:text-slate-300' => $view !== $key,
        ])>
