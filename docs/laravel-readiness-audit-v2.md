...
|---|---------|-------|------------------|
| 1 | DB scaling decision tree | **infra** (code-side prereqs present) | Every external/provider call is a queued job (Horizon + Redis), so writes are already off the request cycle. `config/database.php` has no read/write split — correct for a single MySQL until read volume warrants it. Connection pooling / replicas / Octane are **hosting decisions**, not app code. No code gap. |
