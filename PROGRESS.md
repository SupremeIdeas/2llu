...
- **Naara Gift brand-detail sheet modernized**: restyled to match the eSIM
  ...
  value.
- Full suite green (1481 passed), Pint clean on all touched files,
  Playwright-verified: opening a brand shows the modernized bordered sheet
  with fact-tile denominations; selecting "$25" highlights that tile and
  correctly updates the price bar to "$25.92" (the real markup-applied
  retail price for that face value, confirmed against `GiftCardPricing`).
- Branched off `main` independently of PR #25 (wallet/eSIM-exception audit
  fixes) since these are unrelated front-end features — kept the two PRs
